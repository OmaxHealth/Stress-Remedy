...
function force_page_template($template){
	...
	if (is_front_page() || is_page('home')){//Front Page must be assigned in Setting > Reading
		$new_template = locate_template(array('page-template-offer-v2.php'));
	...
	if (is_page('about-cbd')){
		$new_template = locate_template(array('page-template-about-cbd.php'));
		if (!empty($new_template)) {return $new_template;}
	}
	if (is_page('reviews')){
	...
	if (is_page('lp1004')){
		$new_template = locate_template(array('page-template-lp1004.php'));
		if (!empty($new_template)) {return $new_template;}
	}
	return $template;
}
